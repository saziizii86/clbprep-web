...
const normalizeUploadedFiles = (raw: any) => {
  if (!raw) return {};
...
const getStoredFileUrl = (file: any): string | undefined => {
...
const getStoredText = (file: any): string => {
  if (!file) return "";
...
const getQuestionLevelItem = (
  ...
  // nested format: [[...], [...]]
  if (Array.isArray(arr[sectionIndex])) {
    return arr[sectionIndex][questionIndex];
  }

  // flat format: [...]
  return arr[flatIndex];
};

const buildSectionsFromScenario = (scenario: Scenario): Section[] => {
  const uploadedFiles = normalizeUploadedFiles((scenario as any)?.uploadedFiles);
  const defaultSections = getDefaultSections();

  const baseSections =
    Array.isArray(scenario.sections) && scenario.sections.length > 0
      ? scenario.sections
      : defaultSections;
  ...
  return baseSections.map((section, sectionIndex) => {
    const fallbackSection = defaultSections[sectionIndex];
    const questionCount =
      section?.questions?.length ||
      fallbackSection?.questions?.length ||
      0;

    const builtQuestions: Question[] = Array.from({ length: questionCount }).map((_, questionIndex) => {
      const fallbackQuestion = fallbackSection?.questions?.[questionIndex];
      const originalQuestion = section?.questions?.[questionIndex] || fallbackQuestion;

      const answerRow =
        getQuestionLevelItem(
          uploadedFiles?.questionAnswers,
          sectionIndex,
          questionIndex,
          flatQuestionIndex
        ) || [];
      ...
      const questionTranscript =
        getQuestionLevelItem(
          uploadedFiles?.questionTranscripts,
          sectionIndex,
          questionIndex,
          flatQuestionIndex
        );
      ...
      const builtQuestion: Question = {
        id: originalQuestion?.id || `q-${sectionIndex}-${questionIndex}`,
        questionText: originalQuestion?.questionText || `Question ${flatQuestionIndex + 1}`,
        type: originalQuestion?.type || "multiple-choice",
        options:
          Array.isArray(answerRow) && answerRow.length >= 4
            ? [
                { id: "a", text: answerRow[0] || "" },
                { id: "b", text: answerRow[1] || "" },
                { id: "c", text: answerRow[2] || "" },
                { id: "d", text: answerRow[3] || "" },
              ]
            : originalQuestion?.options || [],
        correctAnswer:
          (Array.isArray(answerRow) && answerRow[4]
            ? String(answerRow[4]).toLowerCase()
            : originalQuestion?.correctAnswer || ""
          ),
        audioUrl:
          getStoredFileUrl(questionAudio) || originalQuestion?.audioUrl || null,
        transcript:
          getStoredText(questionTranscript) || originalQuestion?.transcript || "",
        tip:
          originalQuestion?.tip || "Focus on keywords (numbers, dates, reasons, next steps).",
      };
    ...
    return {
      ...
      title: section?.title || fallbackSection?.title || `Section ${sectionIndex + 1}`,
      audioUrl:
        getStoredFileUrl(uploadedFiles?.sectionAudios?.[sectionIndex]) ||
        section?.audioUrl,
      audioDuration:
        section?.audioDuration ||
        fallbackSection?.audioDuration ||
        60,
      transcript:
        getStoredText(uploadedFiles?.sectionTranscripts?.[sectionIndex]) ||
        section?.transcript ||
        fallbackSection?.transcript ||
        "",
      questions: builtQuestions,
      preparationTime:
        section?.preparationTime ?? fallbackSection?.preparationTime,
    };
  });
};
...
const ListeningPracticeTest: React.FC<ListeningPracticeTestProps> = ({
  ...
  const uploadedFiles = useMemo(
    () => normalizeUploadedFiles((scenario as any)?.uploadedFiles),
    [scenario]
  );

  const sections = useMemo(() => buildSectionsFromScenario(scenario), [scenario]);

  const resolvedContextImage =
    getStoredFileUrl((scenario as any)?.contextImage) ||
    getStoredFileUrl(uploadedFiles?.contextImage);
...
useEffect(() => {
  ...
  // ✅ CONTEXT screen → go back to first page (instructions)
  if (phase === "context") {
...
const Shell: React.FC<{
  ...
  className?: string;
  compact?: boolean;
}> = ({ children, footer, className, compact = false }) => (
  <div className={`${SHELL_BASE} ${compact ? SHELL_COMPACT : SHELL_FIXED} ${className ?? ""}`}>
    <div className={`${SHELL_PAD} flex-1 overflow-y-auto`}>{children}</div>

    {footer ? (
      <div className="px-4 sm:px-6 pb-4 sm:pb-6 pt-0">{footer}</div>
    ) : null}
  </div>
);
...
<div className="text-center mb-4 sm:mb-5">
  ...
  <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1 tracking-tight">
    {scenario.taskName || "Problem Solving"}
  </h2>
...
const renderContext = () => (
  ...
  >
    ...
    <div className="bg-gradient-to-br from-slate-50 to-blue-50/50 rounded-2xl p-6 sm:p-8 mb-6 border border-slate-200/80">
    ...
    {resolvedContextImage && (
      <div className="rounded-2xl overflow-hidden shadow-xl bg-white border border-slate-200 h-64 sm:h-72 flex items-center justify-center">
        <img
          src={resolvedContextImage}
          alt={scenario.title}
...
const renderResults = () => {
  ...
  title={
    phase === "listening"
      ? `${currentSection?.title || "Section"} Transcript`
      : `Question ${getAbsoluteQuestionIndex() + 1} Transcript`
  }
  content={
    phase === "listening"
      ? currentSection?.transcript?.trim() || "No transcript found in database."
      : questionTranscriptText?.trim() || "No question transcript found in database."
  }
/>
