...
const ListeningNewsItemTest: React.FC<Props> = ({ scenario, onBack, onComplete }) => {
  ...
  // ADD after resetTest(), before progressPercentage line
useEffect(() => {
...
const SHELL_FIXED = "min-h-[700px] flex flex-col";
  ...
  const renderListening = () => (
    <div className={`${SHELL_BASE} ${SHELL_FIXED} p-6`}>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
          ...
          <div className="flex items-center gap-4 mb-6">
            <button 
              ...
              className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${hasPlayedAudio ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:scale-105'}`}
            >
              {isPlaying ? <Pause className="w-6 h-6" /> : <Play className="w-6 h-6 ml-0.5" />}
            </button>
            
            <div className="flex-1 flex items-center gap-3">
              <span className="text-sm text-slate-500 font-mono w-12">{formatTime(Math.floor(audioCurrentTime))}</span>
              
              {/* ✅ CHANGED: Interactive slider instead of static progress bar */}
              ...
              <span className="text-sm text-slate-500 font-mono w-12">{formatTime(audioDuration)}</span>
            </div>
          ...
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-200/50">
            <p className="text-sm text-amber-800"><span className="font-bold">Tip:</span> focus on main idea, key details, numbers, and names.</p>
          </div>
        </div>
...
{/* ✅ Bottom navigation pinned (same behavior as Problem Solving) */}
<div className="mt-auto pt-6">
  ...
  <div className="flex gap-4">
    <button
      onClick={handleBack}
      className="flex-1 py-4 bg-slate-100 text-slate-700 rounded-2xl font-semibold hover:bg-slate-200 flex items-center justify-center gap-2"
    >
    ...
    <button
      onClick={handleNext}
      className="flex-1 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold hover:shadow-xl flex items-center justify-center gap-3"
    >
      ...
      {/* ✅ Audio element at the end - like ListeningPracticeTest */}
      {audioUrl && (
        <audio 
          ref={audioRef} 
          src={audioUrl} 
...
<div className="flex items-start justify-between gap-6 mb-8">
  ...
  <button
    onClick={handleNext}
    className="shrink-0 px-5 py-2 rounded-lg font-semibold bg-blue-600 text-white hover:bg-blue-700"
  >
    NEXT
  </button>
</div>
      ...
      <div className="flex-1 space-y-4 divide-y divide-slate-100 overflow-y-auto pr-2">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="relative inline-block">
                <select 
                  ...
                  className={`appearance-none bg-white border rounded-lg px-4 py-2 pr-8 font-medium cursor-pointer min-w-[200px] ${answers[q.id] ? 'border-blue-500 text-slate-900' : 'border-slate-300 text-slate-500'}`}
                >
...
<div className="mt-auto pt-6 border-t border-slate-200 flex justify-end">
  <button
    onClick={handleBack}
...
const renderResults = () => {
  ...
  return (
    <div className={`${SHELL_BASE} ${SHELL_FIXED} p-8 max-w-6xl`}>
