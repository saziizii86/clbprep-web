...
const videoUrl =
  scenario.videoUrl ||
  uploadedFiles?.videoFile?.storageUrl ||
  uploadedFiles?.videoFile?.data;
...
// Get transcript — checks all possible locations the admin may store it
...
useEffect(() => {
  ...
  const loadTranscript = async () => {
    ...
    const isTxtFile =
      ...
      source.name.toLowerCase().endsWith(".txt");
...
// Shell sizing — matches PracticeTest & NewsItem
const SHELL_BASE = 'bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-200 w-full max-w-4xl mx-auto';
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="text-center mb-4 sm:mb-5">
          <div className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl mb-4 shadow-xl shadow-blue-500/25">
            {hasVideo ? <Video className="w-7 h-7 sm:w-8 sm:h-8 text-white" /> : <Headphones className="w-7 h-7 sm:w-8 sm:h-8 text-white" />}
          </div>
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1 tracking-tight">Discussion</h2>
          <p className="text-slate-500 text-sm sm:text-base">{scenario.title?.replace(/_/g, ' ') || 'Listening Practice'}</p>
        </div>
...
const renderQuestion = () => {
  ...
  return (
    <div className={`${SHELL_BASE} ${SHELL_FIXED} ${SHELL_PAD}`}>
      ...
      {/* Questions list — scrollable, takes remaining space */}
              ...
              <div className="relative w-full sm:min-w-[240px] sm:w-auto">
                <select
                  ...
                  className={`w-full appearance-none bg-white border rounded-xl px-3 sm:px-4 py-2 pr-8 font-medium cursor-pointer text-sm sm:text-base ${
                    answers[q.id] ? 'border-blue-500 text-slate-900' : 'border-slate-300 text-slate-500'
                  }`}
                >
