...
const seekToPercent = (pct: number) => {
  ...
  const renderInstructions = () => (
    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-slate-200 max-w-5xl mx-auto min-h-[700px] flex flex-col">
      ...
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-slate-900 mb-2">Listening to Viewpoints</h2>
      ...
      <button onClick={handleNext} className="mt-auto w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold text-lg hover:shadow-xl hover:shadow-blue-500/25 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3">
  ...
  const renderListening = () => (
    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-slate-200 max-w-5xl mx-auto min-h-[700px] flex flex-col">
      ...
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Listen to the Viewpoints</h2>
        <p className="text-slate-500">{scenario.instructions || 'Listen carefully to the speakers\' viewpoints.'}</p>
      </div>
      <div className="bg-gradient-to-br from-slate-50 to-blue-50/50 rounded-2xl p-6 mb-6 border border-slate-200/80">
        <div className="flex items-center gap-4">
          <button onClick={togglePlayPause} disabled={hasPlayedAudio} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${hasPlayedAudio ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white hover:shadow-xl hover:shadow-blue-500/25'}`}>
...
<div className="flex-1 flex items-center gap-3">
  ...
  <span className="text-sm text-slate-500 font-mono w-12">
    {formatTime(audioDuration)}
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-200/50 mt-4"><p className="text-sm text-amber-800"><span className="font-bold">Tip:</span> pay attention to different speakers' opinions and their reasons.</p></div>
      </div>
      ...
      <p className="text-sm text-slate-400 my-6 text-center">In the real test, the audio plays once and no transcript is available.</p>
      <div className="mt-auto flex gap-4">

        <button onClick={handleBack} className="flex-1 py-4 bg-slate-100 text-slate-700 rounded-2xl font-semibold hover:bg-slate-200 flex items-center justify-center gap-2"><ArrowLeft className="w-5 h-5" /> Back</button>
        <button onClick={handleNext} className="flex-1 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold hover:shadow-xl flex items-center justify-center gap-3">Next <ArrowRight className="w-5 h-5" /></button>
      </div>
    </div>
  );
  ...
  const renderQuestions = () => (
    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-slate-200 max-w-5xl mx-auto min-h-[700px] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6">
        <div><h2 className="text-xl font-bold text-slate-900">{scenario.title?.replace(/_/g, ' ')} - Listening Part 6: Listening to Viewpoints</h2><p className="text-slate-500 text-sm mt-1">Choose the best answer to each question from the drop-down menu (▼).</p></div>
