// D:\projects\celpip-master\src\pages\Mock\L_T5_MockTest.tsx
// Listening Part 5 (Video + Dropdown Questions on one page)
// Independent practice UI (not affiliated with any official testing organization).

import React, { useEffect, useMemo, useRef, useState } from "react";
import { useLocation, useParams } from "react-router-dom";
import { AlertCircle, Clock, FileText, Info, PlayCircle, X } from "lucide-react";
...
type UploadedFiles = {
  ...
  // ✅ Part 5: video instead of audio
  // Prefer sectionVideos[0], but also support videoUrl string for compatibility.
videoFile?: SerializedFile;
sectionVideos?: SerializedFile[];
videoUrl?: string;
...
function isPart5VideoMock(m: Partial<MaterialDoc>) {
...
export default function L_T5_MockTest({
  onComplete,
  onBack,
  ...
  // Part 5: all dropdown questions on one page with a timer.
...
const sectionVideos = (uploaded.sectionVideos ?? []).filter(Boolean) as SerializedFile[];

// ✅ Prefer admin’s uploadedFiles.videoFile, else fall back to sectionVideos[0]
const videoFile = (uploaded.videoFile ?? sectionVideos?.[0] ?? null) as SerializedFile;

const videoFileUrl = useFileUrl(videoFile);

const videoUrl = useMemo(() => {
  // Prefer a direct hosted URL if provided, otherwise use the serialized file URL (data/storageUrl)
  const direct = typeof uploaded.videoUrl === "string" ? uploaded.videoUrl.trim() : "";
  return direct || videoFileUrl || null;
}, [uploaded.videoUrl, videoFileUrl]);
...
const goNext = async () => {
  ...
  const handleVideoPlay = () => {
    ...
    if (playedOnceRef.current && v.currentTime === 0) {
      v.pause();
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 flex items-center justify-center p-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm max-w-xl w-full">
          ...
          <div className="text-sm text-gray-700 leading-6">
            Could not load a Listening mock for Part 5 (Video). Please check your database record.
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 p-6">
      ...
      <PracticeFrame
        ...
        onNext={goNext}
        onBack={goBack}
        nextLabel={step.kind === "questions-all" ? "NEXT" : "NEXT"}
        ...
        {step.kind === "intro-bullets" ? (
          <div className="p-10">
            <div className="mx-auto max-w-3xl">
              <div className="text-lg font-semibold text-gray-900 mb-4">Watch the conversation</div>

              <div className="bg-white border border-gray-200 rounded-2xl p-6">
                <ul className="text-sm text-gray-700 leading-7 list-disc pl-5 space-y-3">
                  <li>You will watch one short video. It appears only once in this practice.</li>
                  <li>After the video, all questions will appear on one page.</li>
            ...
            <div className="mx-auto max-w-3xl">
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-6 text-sm text-gray-700 leading-7">
                {uploaded.instructions?.trim()
                  ...
                  : "Watch the video once. Then answer the questions by choosing options from the dropdown menus."}
              </div>
            </div>
          </div>
        ) : null}
          ...
          <div className="p-10">
            <div className="mx-auto max-w-4xl">
              ...
              <div className="flex justify-center">
                <div className="w-[min(820px,92vw)] rounded-2xl bg-gradient-to-br from-indigo-100 to-purple-100 p-6 border border-indigo-200">
                  {videoUrl ? (
                    <video
                      ref={videoRef}
                      key={videoUrl}
                      ...
                      onEnded={() => {
                        ...
                        // Keep last frame, but prevent restart from 0 by pausing.
                        const v = videoRef.current;
                        if (v) v.pause();
                      }}
                    >
                    ...
                    <div className="bg-white border border-gray-300 rounded p-4 text-sm text-gray-600">
                      No video uploaded. Please set uploadedFiles.videoUrl OR uploadedFiles.sectionVideos[0].
                    </div>
                  )}
              ...
              {totalQuestions <= 0 ? (
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6">
                  ...
                  <div className="mt-6 text-xs text-gray-500">
                    Tip: Make sure every dropdown has a selection before clicking NEXT.
