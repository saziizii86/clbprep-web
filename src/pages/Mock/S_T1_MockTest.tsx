// src/pages/Mock/S_T1_MockTest.tsx  –  Speaking Task 1: Giving Advice
...
const TASK = 1, TASK_ID = "part1", PREP_TIME = 30;
const TASK_LABEL = "Speaking Task 1: Giving Advice";
const NEXT_ROUTE = "/speaking/2"; // Route to next task

export default function S_T1_MockTest({ material: pre, mockSet: preSet, onComplete, onBack }: {
  ...
  // Promise that resolves when recording stops AND Whisper finishes
...
const apply = useCallback(async (doc: any) => {
    ...
    // Save prompt immediately to localStorage AND DB
    ...
    // Save to DB (don't block UI if it fails)
  ...
  // Load material from database if not provided
  useEffect(() => {
    if (pre) return;
    let cancelled = false;
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      rec.onstop = async () => {
        ...
        // Await Whisper transcription BEFORE resolving the promise
  ...
  // Handle Next: stop recording, WAIT for Whisper, THEN navigate
  const handleNext = async () => {
    // If still in prep, just skip to next (no recording yet)
    if (phase === "prep") {
      clearInterval(prepRef.current);
    ...
    // If recording, stop it and wait for Whisper to finish
    if (recRef.current?.state === "recording") {
      recRef.current.stop();
    }

    // Wait for the onstop + Whisper to complete
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-50 p-6">
      ...
      >
        <div className="p-6 space-y-6">
          ...
          {/* Images (centered, above countdown) */}
          {hasImages && (
  ...
  <div className={`${hasImages ? "md:col-span-1 flex justify-end" : "flex justify-center"}`}>
    {phase === "prep" ? (
      ...
      <div className="bg-red-50 rounded-lg border border-red-200 shadow-sm p-3 w-[240px] text-center">
        ...
        <p className="text-[10px] text-gray-600 font-medium">{recTime}s / {maxRec}s</p>
      </div>
    ) : phase === "transcribing" ? (
