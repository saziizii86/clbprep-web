...
export const getMaterials = async (): Promise<Material[]> => {
  ...
  try {
    const response = await databases.listDocuments(
      ...
      [
        Query.select([
          "$id", "title", "skill", "task", "taskId",
          "type", "status", "date", "questions",
          "description", "isMock", "mockSet", "mockOrder"
        ]),
...
// Fetch a single full material including uploadedFiles — call this only when a scenario is opened
export const getMaterialById = async (id: string): Promise<Material | null> => {
  try {
    const doc = await databases.getDocument(DATABASE_ID, MATERIALS_COLLECTION_ID, id);
