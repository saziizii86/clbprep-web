...
export const getMaterials = async (): Promise<Material[]> => {
  try {
    const response = await databases.listDocuments(
      ...
      [
        //Query.equal("status", "Published"),   // ← Uses your status_idx
        //Query.orderAsc("mockOrder"),          // nice sorting for mocks
		Query.orderDesc("$createdAt"),
        Query.limit(500)
      ]
    );

    console.log(`📡 Fetched ${response.documents.length} PUBLISHED materials (indexed & filtered)`);
